...
from inputters.bert_field import BertField
import torch
...
def make_text_iterator_from_file(path):
  lines_count = rawincount(path)
  with codecs.open(path, "r", "utf-8") as corpus_file:
    for line in tqdm(corpus_file,total=lines_count):
      yield line
...
def make_features(batch, side):
  ...
  Returns:
      ...
      of size (len x batch).
  """
  assert side in ['src', 'bert_src', 'tgt', 'tgt2']
...
class Dataset(torchtext.data.Dataset):
  ...
  def make_examples(text_iter, truncate, side, task_type):
    for i, line in enumerate(text_iter):
      ...
      if side == "src":
        example_dict[f"bert_src"] = tuple(words)
      yield example_dict
