...
def build_vocab(train_dataset_files, fields, share_vocab,
  ...
  for _, path in enumerate(train_dataset_files):
    ...
    for ex in dataset.examples:
      for k in fields:
        ...
        if not fields[k].sequential or k == "bert_src":
          continue
  ...
  if vocab_tgt_file:
    logger.info('update tgt vocab')
    logger.info('orin {}'.format(len(counter["tgt"])))
    with open(vocab_file, mode='r', encoding='utf-8') as vf:
      for line in vf:
        vocab = line.strip().split()
        counter["tgt"].update(vocab)
    logger.info('with vocab {}'.format(len(counter["tgt"])))


  merged_vocab = merge_vocabs(
        [fields["tgt"].vocab, fields["tgt2"].vocab],
        vocab_size=tgt_vocab_size,
        min_frequency=tgt_words_min_frequency)
...
def build_save_vocab(train_dataset, fields, opt):
  ...
  fields = build_vocab(train_dataset, fields,
                                 ...
                                 opt.vocab_src_file, opt.vocab_tgt_file)
