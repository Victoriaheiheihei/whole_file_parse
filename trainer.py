  ...
  def validate(self, valid_iter, task_type='task'):
    ...
    with torch.no_grad():
      for batch in valid_iter:
        ...
        bert_src = make_features(batch, 'bert_src')
        ...
        outputs, attns = self.model(src, bert_src, tgt, src_lengths, task_type=task_type)
      ...
      for batch in true_batchs:
          ...
          for j in range(0, target_size-1, trunc_size):
              ...
              if self.grad_accum_count == 1:
                  self.model.zero_grad()
              outputs, attns = \
                  self.model(src, bert_src, tgt, src_lengths, task_type=task_type)
