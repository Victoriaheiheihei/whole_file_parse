...
class Translator(object):
  ...
  def translate_batch(self, batch):
    ...
    with torch.no_grad():
      ...
      src_bert_seq = make_features(batch, 'bert_src')
      # src: (seq_len_src, batch_size)
      src_emb, src_enc, _ = self.model.encoder(src_seq,src_bert_seq)
      # src_emb: (seq_len_src, batch_size, emb_size)
      # src_end: (seq_len_src, batch_size, hid_size)

      self.model.decoder.init_state(src_bert_seq.transpose(0, 1), src_enc)
