...
class JointModel(nn.Module):
  ...
  def forward(self, src, bert_src, tgt, lengths, task_type):
    tgt = tgt[:-1]  # exclude last target from inputs

    _, memory_bank, lengths = self.encoder(src, bert_src, lengths)

    self.decoder.init_state(bert_src.transpose(0, 1), memory_bank)
...
class NMTModel(nn.Module):
  ...
  def forward(self, src, bert_src, tgt, lengths):
    tgt = tgt[:-1]  # exclude last target from inputs
...
def build_base_model(model_opt, fields, gpu, checkpoint=None):
  ...
  if checkpoint is not None:
    ...
    encoder.reset_parameters(model_opt)
